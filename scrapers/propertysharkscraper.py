...
REQUEST_GET_PROXIES_USE = settings.PROPERTYSHARK_REQUEST_GET_PROXIES_USE
# Will use proxy service specified in scraper settings for request.get() method
REQUEST_GET_PROXIES_SERVICE = settings.PROPERTYSHARK_REQUEST_GET_PROXIES_SERVICE
...
with open('data.csv', 'a', newline='', encoding = 'utf8') as csvfile:
    ...
    base_link = settings.propertyshark_url
    ...
    number_of_pages = settings.propertyshark_pages
    ...
    for page in range(number_of_pages):
        print(f'PAGE {next_page_url}, STATUS CODE {str(status_code)}, PROPERTYSHARK.COM, ITEMS SCRAPED {items_scraped}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}')
        csv_writer.writerow([f'PAGE {next_page_url}', f'STATUS CODE {str(status_code)}', f'PROPERTYSHARK.COM', f'ITEMS SCRAPED {items_scraped}', f'PAGE {on_page_num}', f'PAGE {on_page_num}', f'PAGE {on_page_num}',f'PAGE {on_page_num}', f'PAGE {on_page_num}', f'PAGE {on_page_num}', f'PAGE {on_page_num}'])
        ...
        for listing_url_container in listing_url_containers:
            try:
                if listing_url_container != None:
                    listing_url = "https://www.propertyshark.com" + listing_url_container.a.get('href')
