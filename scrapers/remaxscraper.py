...
REQUEST_GET_PROXIES_USE = settings.REMAX_REQUEST_GET_PROXIES_USE
# Will use proxy service specified in scraper settings for request.get() method
REQUEST_GET_PROXIES_SERVICE = settings.REMAX_REQUEST_GET_PROXIES_SERVICE
...
with open('data.csv', 'a', newline='', encoding = 'utf8') as csvfile:
    ...
    number_of_pages = settings.remax_pages
    ...
    for page in range(number_of_pages):
        print(f'PAGE {next_page_url}, STATUS CODE {str(status_code)}, REMAX.COM, ITEMS SCRAPED {items_scraped}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}, PAGE {on_page_num}')
        csv_writer.writerow([f'PAGE {next_page_url}', f'STATUS CODE {str(status_code)}', f'REMAX.COM', f'ITEMS SCRAPED {items_scraped}', f'PAGE {on_page_num}', f'PAGE {on_page_num}', f'PAGE {on_page_num}',f'PAGE {on_page_num}', f'PAGE {on_page_num}', f'PAGE {on_page_num}', f'PAGE {on_page_num}'])
        soup = BeautifulSoup(html_text, 'lxml')

        listings = soup.find_all('div', class_ = 'listings-card')
        
        for listing in listings:
            try:
                listing_url = "https://www.remax.com" + wscrape.verify_not_nonetype(listing.find('div',  class_ = 'card-full-address cursor-pointer'), "obj.a.get('href')")
                ...
                property_description = wscrape.verify_not_nonetype(individual_listing.find('div', class_ = 'listing-detail-read-more'), "obj.text")
                
                address = wscrape.if_nonetype_str(wscrape.verify_not_nonetype(listing.find('div',  class_ = 'card-full-address cursor-pointer'), "obj.a.h5.text")) + wscrape.if_nonetype_str(wscrape.verify_not_nonetype(listing.find('div',  class_ = 'card-full-address cursor-pointer'), "obj.a.h5.span.text"))
                purchase_price = wscrape.remove_ws_nl(wscrape.verify_not_nonetype(individual_listing.find('div', class_ = 'listing-detail-header-row md:ml-auto'), "obj.h4.text"))
